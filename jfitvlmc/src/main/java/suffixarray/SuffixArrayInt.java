...
public class SuffixArrayInt {
    ...
    public static class SuffixInt implements Comparable<SuffixInt> {
        ...
        public int compareTo(SuffixInt that) {
            ...
            for (int i = 0; i < n; i++) {
                if (this.symbolAt(i) < that.symbolAt(i)) return -1;
                if (this.symbolAt(i) > that.symbolAt(i)) return +1;
            }
    ...
    public void buildLCPArraykasai() {
        ...
        for (int i = 0; i < n; i++) invSuff[this.suffixes[i].getIndex()] = i;
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (i + k < n && j + k < n && this.data.get(i + k) == this.data.get(j + k)) k++;
    ...
    private static int lcpSuffix(SuffixInt s, SuffixInt t) {
        ...
        for (int i = 0; i < n; i++) {
            if (!s.symbolAt(i).equals(t.symbolAt(i))) return i;
        }
        return n;
    }
    ...
    /**
     * Returns the number of suffixes strictly less than the {@code query} string. We note that
     * {@code rank(select(i))} equals {@code i} for each {@code i} between 0 and <em>n</em>-1.
     *
    ...
    private static int compare(ArrayList<Integer> query, SuffixInt suffix) {
        ...
        for (int i = 0; i < n; i++) {
            if (query.get(i) < suffix.symbolAt(i)) return -1;
            if (query.get(i) > suffix.symbolAt(i)) return +1;
        }
    ...
    private static int[] myCompare(ArrayList<Integer> query, SuffixInt suffix, int k) {
        ...
        int i = k;
        for (; i < limit; i++) {
            if (query.get(i) < suffix.symbolAt(i)) return new int[] {-1, i};
            if (query.get(i) > suffix.symbolAt(i)) return new int[] {1, i};
        }
        // query is longer than suffix but all shared symbols match → query > suffix
        if (query.size() > suffix.length()) {
    ...
    public int search(int low, int high, ArrayList<Integer> pat) {
        if (high >= low) {
            ...
            if (res == 0) return mid;
            else if (res > 0) return this.search((mid + 1), high, pat);
            else return this.search(low, (mid - 1), pat);
        }
        return -1;
    }
    ...
    public int[] count(ArrayList<Integer> pat) {
        ...
        if ((j - i + 1) < 0) {
            throw new IllegalStateException(
                    "negative number of occurrences: first=" + i + " last=" + j);
        }
    ...
    public int first(int low, int high, ArrayList<Integer> pat) {
        if (high >= low) {
            ...
            if ((mid == 0 || SuffixArrayInt.myCompare(pat, this.suffixes[mid - 1]) > 0) && res == 0)
                return mid;
            else if (res > 0) return this.first((mid + 1), high, pat);
            else return this.first(low, (mid - 1), pat);
        }
        return -1;
    }
    ...
    public int last(int low, int high, ArrayList<Integer> pat) {
        if (high >= low) {
            ...
            if ((mid == this.length() - 1
                            || SuffixArrayInt.myCompare(pat, this.suffixes[mid + 1]) < 0)
                    && res == 0) {
                return mid;
            } else if (res < 0) {
