...
public class SuffixArray {
    ...
    private static int[] myCompare(String query, Suffix suffix, int k) {
        ...
        try {
            for (; i < query.length(); i++) {
                if (query.charAt(i) < suffix.charAt(i)) return new int[] {-1, i};
                if (query.charAt(i) > suffix.charAt(i)) return new int[] {1, i};
            }
        } catch (StringIndexOutOfBoundsException e) {
            // significa che query è più lunga di suffix ma i primi caratteri coincidono
    ...
    public int last(int low, int high, String pat) {
        if (high >= low) {
            int mid = (low + high) / 2; /* low + (high - low)/2; */
            int res = SuffixArray.myCompare(pat, this.suffixes[mid]);
            if ((mid == this.length() - 1 || SuffixArray.myCompare(pat, this.suffixes[mid + 1]) < 0)
                    && res == 0) {
                return mid;
            } else if (res < 0) {
    ...
    public int last(String p) {
        ...
        while (true) {
            if (r >= l) {
                ...
                if (res[0] == 0
                        && (c == this.length() - 1
                                || SuffixArray.myCompare(p, this.suffixes[c + 1]) < 0)) {
                    return c;
                }
