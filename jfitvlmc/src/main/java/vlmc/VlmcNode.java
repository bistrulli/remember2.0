...
public class VlmcNode implements Cloneable {
    ...
    public void prune() {
        ...
        if (this.getParent().getDist() == null)
            this.parent.setDist(
                    EcfNavigator.createNextSymbolDistribution(this.getParent().getCtx()));

        if (!"root".equals(this.parent.getLabel())
                && (this.KullbackLeibler() * this.dist.totalCtx <= fitVlmc.cutoff
                        || this.dist.totalCtx < fitVlmc.k)) {
    ...
    private double maxDifference() {
        ...
        for (String symbol : this.getDist().symbols) {
            if (Math.abs(
                            this.dist.getProbBySymbol(symbol)
                                    - this.getParent().getDist().getProbBySymbol(symbol))
                    >= maxDifference) {
                maxDifference =
                        Math.abs(
                                this.dist.getProbBySymbol(symbol)
    ...
    public void computeOrder(int order) {
        if (!(this instanceof VlmcRoot)) order++;

        if (VlmcRoot.order < order) VlmcRoot.order = order;
